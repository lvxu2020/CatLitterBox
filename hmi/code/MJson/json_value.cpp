...
static inline double integerToDouble(MJson::UInt64 value) {
...
static inline char* duplicateStringValue(const char* value, size_t length) {
    ...
    if (newString == NULL) {
        throwRuntimeError("in MJson::Value::duplicateStringValue(): "
...
static inline char* duplicateAndPrefixStringValue(const char* value,
    ...
    JSON_ASSERT_MESSAGE(length <= static_cast<unsigned>(Value::maxInt) -
                        sizeof(unsigned) - 1U,
                        "in MJson::Value::duplicateAndPrefixStringValue(): "
                        "length too big for prefixing");
    unsigned actualLength = length + static_cast<unsigned>(sizeof(unsigned)) + 1U;
    char* newString = static_cast<char*>(malloc(actualLength));
    if (newString == 0) {
        throwRuntimeError("in MJson::Value::duplicateAndPrefixStringValue(): "
                          "Failed to allocate string value buffer");
    }
...
void Value::CommentInfo::setComment(const char* text, size_t len) {
    ...
    JSON_ASSERT_MESSAGE(
                text[0] == '\0' || text[0] == '/',
            "in MJson::Value::setComment(): Comments must start with /");
...
bool Value::operator==(const Value& other) const {
    ...
    // GCC 2.95.3 says:
    // attempt to take address of bit-field structure member `MJson::Value::type_'
...
const char* Value::asCString() const {
    JSON_ASSERT_MESSAGE(type_ == stringValue,
                        "in MJson::Value::asCString(): requires stringValue");
    if (value_.string_ == 0)
        return 0;
    unsigned this_len;
    char const* this_str;
...
void Value::clear() {
    JSON_ASSERT_MESSAGE(type_ == nullValue || type_ == arrayValue ||
                        type_ == objectValue,
                        "in MJson::Value::clear(): requires complex value");
    start_ = 0;
    limit_ = 0;
    switch (type_) {
    case arrayValue:
...
void Value::resize(ArrayIndex newSize) {
    JSON_ASSERT_MESSAGE(type_ == nullValue || type_ == arrayValue,
                        "in MJson::Value::resize(): requires arrayValue");
    if (type_ == nullValue)
...
Value& Value::operator[](ArrayIndex index) {
    JSON_ASSERT_MESSAGE(
                ...
                "in MJson::Value::operator[](ArrayIndex): requires arrayValue");
    if (type_ == nullValue)
        *this = Value(arrayValue);
    CZString key(index);
...
Value& Value::operator[](int index) {
    JSON_ASSERT_MESSAGE(
                index >= 0,
                "in MJson::Value::operator[](int index): index cannot be negative");
...
const Value& Value::operator[](ArrayIndex index) const {
    JSON_ASSERT_MESSAGE(
                type_ == nullValue || type_ == arrayValue,
                "in MJson::Value::operator[](ArrayIndex)const: requires arrayValue");
    if (type_ == nullValue)
...
const Value& Value::operator[](int index) const {
    JSON_ASSERT_MESSAGE(
                index >= 0,
                "in MJson::Value::operator[](int index) const: index cannot be negative");
    return (*this)[ArrayIndex(index)];
}
...
Value& Value::resolveReference(const char* key) {
    JSON_ASSERT_MESSAGE(
                ...
                "in MJson::Value::resolveReference(): requires objectValue");
    if (type_ == nullValue)
...
Value& Value::resolveReference(char const* key, char const* end) {
    JSON_ASSERT_MESSAGE(
                type_ == nullValue || type_ == objectValue,
                "in MJson::Value::resolveReference(key, end): requires objectValue");
    if (type_ == nullValue)
        *this = Value(objectValue);
...
Value const* Value::find(char const* begin, char const* end) const {
    JSON_ASSERT_MESSAGE(type_ == nullValue || type_ == objectValue,
                        "in MJson::Value::find(key, end, found): requires "
...
void Value::removeMember(const char* key) {
    JSON_ASSERT_MESSAGE(type_ == nullValue || type_ == objectValue,
                        "in MJson::Value::removeMember(): requires objectValue");
    if (type_ == nullValue)
        return;
...
Value::Members Value::getMemberNames() const {
    JSON_ASSERT_MESSAGE(
                type_ == nullValue || type_ == objectValue,
                "in MJson::Value::getMemberNames(), value must be objectValue");
    if (type_ == nullValue)
...
JSONCPP_STRING Value::toStyledString() const {
    ...
    out += MJson::writeString(builder, *this);
    out += '\n';
